 ...
 */
export const listenerDownload = (
  event: Event,
  item: DownloadItem,
  webContents: WebContents
): void => {
  // 新建下载为空时，会执行 electron 默认的下载处理
  if (newDownloadItem == null) return
  ...
  addDownloadItem({
    ...
    item.on('done', (e, state) => {
      ...
      if (state === 'completed') {
        ...
        if (saveFilePath.endsWith('.download')) { saveFilePath = saveFilePath.substr(0, downloadItem.path.length - 9) }
        let index = 0
        ...
        if (arr != null) fileNameWithoutExt = fileNameWithoutExt.substr(0, fileNameWithoutExt.length - 3).trim()
        ...
        if (process.platform === 'darwin') { app.dock.downloadFinished(downloadItem.path) }
      }
 ...
 */
const openFileDialog = async (oldPath: string = app.getPath('downloads')): Promise<string> => {
  if (win == null) return oldPath
 ...
 * @param item - 下载项
 */
const pauseOrResume = (item: IDownloadFile): IDownloadFile => {
 ...
 * @param index - 下载项的下标
 */
const removeDownloadItem = (item: IDownloadFile, index: number): IDownloadFile => {
  const sourceItem = getDownloadItem(downloadItemData, item.id)
 ...
 * 清空已完成的下载项
 */
const clearDownloadDone = (): IDownloadFile[] => {
...
const getDownloadItemCount = (): number => {
...
const listenerEvent = (): void => {
  ...
  // 选择保存位置对话框
  ipcMain.handle('download:openFileDialog', async (event, oldPath?: string) =>
    await openFileDialog(oldPath)
  )
