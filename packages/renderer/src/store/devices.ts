  ...
  'device',
  {
    ...
    actions: {
      async mergeSearchResult(devices) {
        const defaultAdbPath = await window.ipcRenderer.invoke(
        ...
        for (const device of devices) {
          ...
          if (origin == null) {
            ...
            ['disconnected', 'unknown', 'available'].includes(origin.status)
          ) {
            // 曾今连过，更新
      ...
      async wakeUpDevice(uuid) {
        ...
        const wakeUpMessage = showMessage(
          `正在尝试启动设备 ${origin.displayName as string}`,
          {
            type: 'loading',
            duration: 0,
          }
        ...
        for (let i = 0; i < connectRetry; i++) {
          ...
          logger.info(
            `wait ${to}ms for emulator ${origin.displayName as string} start`
          )
